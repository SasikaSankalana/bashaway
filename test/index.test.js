...
test("all organization repos must be cloned properly", async () => {
  //   await exec("bash ./execute.sh sliit-foss ./out");
  //   await exec('bash -c "./execute.sh sliit-foss ./out"');
  //   await exec('bash -c "./execute.sh sliit-foss ./out"');
  ...
  for (const repo of repos) {
    ...
    const actual = await exec(
      'bash -c "basename $(git remote get-url origin)"'
    );
    expect(actual.trim().replace(new RegExp(".git$"), "")).toStrictEqual(
      repo.name
    );
    process.chdir(dir);
  }
